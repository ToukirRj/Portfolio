...
const RightOffCanvas = ({ isOpen, onClose }) => {
  ...
  return (
    <>
        <div className={`fixed z-[9999999] top-0 right-0 md:w-[560px] w-[96%] h-[100vh] bg-[#faf4e7] shadow-lg rounded-l-[25px] transition-transform transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className={`fixed z-[999] top-0 left-0 w-full h-[100vh] bg-[#1F2937] bg-opacity-70 ${isOpen ? 'block' : 'hidden'} transition-opacity duration-300 ease-in-out`} onClick={onClose}></div>
    </>
  );
};
